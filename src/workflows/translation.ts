...
interface WorkflowPayload {
  job: { id: string; workspace_id: string; project_id: string; version_id: string; source_word_count: number };
...
async function loadWorkflowPayload(jobId: string): Promise<WorkflowPayload> {
  "use step";
  const admin = createAdminClient();
  const { data: job, error: jobError } = await admin.from("translation_jobs").select("id,workspace_id,project_id,version_id,source_word_count").eq("id", jobId).single();
  ...
  const [{ data: segments, error: segmentError }, { data: glossary }, { data: memory }, { data: globalMemory }] = await Promise.all([
    ...
    admin.from("translation_memory").select("source_text,target_text").eq("workspace_id", job.workspace_id).eq("approved", true).limit(40),
...
async function finalizeDocument(jobId: string, payload: WorkflowPayload, successfulWords: number) {
  ...
  const warnings = layoutWarningsForSegments((segments ?? []).map((segment) => ({ id: segment.node_id ?? segment.id, source: segment.source_text, translation: segment.translated_text ?? "", page: pageByNode.get(segment.node_id ?? "") ?? 1 })));

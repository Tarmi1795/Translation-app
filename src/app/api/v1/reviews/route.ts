...
export async function POST(request: Request) {
  try {
    ...
    if (input.action === "approve") {
      ...
      const { data: segments } = await admin.from("segments").select("id,source_text,translated_text").eq("project_id", project.id).not("translated_text", "is", null);
      ...
      await Promise.all([
        ...
        admin.from("segments").update({ status: "approved" }).eq("project_id", project.id),

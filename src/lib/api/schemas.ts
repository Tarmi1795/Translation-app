...
export const estimateSchema = z.object({ workspaceId: uuid, projectId: uuid, documentId: uuid.optional(), text: z.string().max(2_000_000).optional(), direction: z.enum(LANGUAGE_DIRECTIONS) }).refine((value) => Boolean(value.documentId || value.text?.trim()), "A document or source text is required.");
